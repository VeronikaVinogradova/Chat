...
const INCIDENT_KERNEL = [
  ...
  "не приходит", "нет звука", "не звонит", "не слышн",
...
export function routingAction(intent: Intent, direction: Direction): string {
  switch (intent) {
    case "incident":
      return "Распознал инцидент. Заведу обращение — выберите следующий шаг в чате.";
    case "review":
      return "Передал отзыв в сервис сбора отзывов. Ответственный сотрудник ответит офлайн — здесь или на почту.";
    case "suggestion":
      return "Распознал продуктовое предложение. Помогу оформить его для команды разработки.";
    default:
